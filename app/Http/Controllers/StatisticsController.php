...
class StatisticsController extends Controller
{
    function getStatistikPenyetoran(Request $request){
        ...
        for ($i=0;$i<=$numberOfWeeksShown;$i++){
            ...
            $setor = DB::table('setor')->select('nama_item', DB::raw('sum(berat) as total_massa'))
            ...
            $count = $setor->groupBy('nama_item')->get();

            $jumlah_massa_data[] = $count;

            // Jumlah uang penyetoran tiap jenis sampah per minggu
            $setor = DB::table('setor')->select('nama_item', DB::raw('sum(harga) as total_harga'))
                        ->where('tanggal', '<=', $end_date->copy()->subWeeks($i))
                        ->where('tanggal', '>', $end_date->copy()->subWeeks($i+1));
    ...
    //penjualan
    function getStatistikPenjualan(Request $request){
        $numberOfWeeksShown = 6;
        ...
        for ($i=0;$i<=$numberOfWeeksShown;$i++){
            // Jumlah penjualan per minggu
            $jual = Jual::where('tanggal', '<=', $end_date->copy()->subWeeks($i))
                        ->where('tanggal', '>', $end_date->copy()->subWeeks($i+1));
            ...
            $jumlah_jual_data[] = $count;
            $labels[] = $end_date->copy()->subWeeks($i)->format('d M Y');


            // Jumlah massa penjualan tiap jenis sampah per minggu
            ...
            // Jumlah uang penjualan tiap jenis sampah per minggu
        ...
        $jumlah_jual_data = json_encode(array_reverse($jumlah_jual_data));
        $labels = json_encode(array_reverse($labels));

        $cabang = Cabang::get();

        return view('statistics.indexJual', compact('jumlah_jual_data', 'dataLabels', 'labels', 'default_start_date', 'default_end_date', 'cabang', 'default_cabang', 'result_uang', 'result_massa'));
    }

    function getJumlahPenjualan(Request $request){
        ...
        return view('statistics.penjualan', compact('data', 'labels', 'default_start_date', 'default_end_date', 'cabang', 'default_cabang'));
    }

    function getJumlahMassaTiapSampahTerjual(Request $request){
        $numberOfWeeksShown = 6;
        ...
        return view('statistics.massaJual', compact('data', 'dataLabels', 'labels', 'default_start_date', 'default_end_date', 'cabang', 'default_cabang', 'result'));
    }

    function getJumlahUangTiapSampahTerjual(Request $request){
        $numberOfWeeksShown = 6;
        ...
        return view('statistics.uangJual', compact('data', 'dataLabels', 'labels', 'default_start_date', 'default_end_date', 'cabang', 'default_cabang', 'result'));
    }
